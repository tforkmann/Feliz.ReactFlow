...
[<Erase>]
type markerEdge =
    static member inline source(source: int) = Interop.mkAttr "source" source

    static member inline markerStart(markerStart: obj) =
        Interop.mkAttr "markerStart" markerStart

    static member inline markerEnd(markerEnd: obj) = Interop.mkAttr "markerEnd" markerEnd
...
type ReactFlow =
    ...
    static member inline node(props: INodeProp seq) : INode = !!(createObj !!props)

    static member inline edge(props: IEdgeProp seq) : IEdge = !!(createObj !!props)
    ...
    static member inline style(style: string) : IReactFlowProp = Interop.mkReactFlowProp "style" style
    ...
    static member inline applyNodeChanges(handler: NodeChanges -> Node []-> unit) : IReactFlowProp =
        !!("applyNodeChanges" ==> System.Func<_, _, _> handler)
    static member inline applyEdgeChanges(handler: NodeChanges -> Node []-> unit) : IReactFlowProp =
        !!("applyEdgeChanges" ==> System.Func<_, _, _> handler)
    ...
    static member inline onNodesChange(handler: NodeChanges -> unit) : IReactFlowProp = !!("onNodesChange" ==> handler)

    static member inline onConnect(handler: OnConnectParams -> unit) : IReactFlowProp = !!("onConnect" ==> handler)
    ...
    static member inline onConnectStop(handler: Event -> unit) : IReactFlowProp = !!("onConnectStop" ==> handler)

    static member inline onConnectEnd(handler: Event -> unit) : IReactFlowProp = !!("onConnectEnd" ==> handler)

    static member inline onEdgesChange(handler: EdgeChanges -> unit) : IReactFlowProp = !!("onEdgesChange" ==> handler)
    ...
    static member inline onMove(flowTransform: unit) : IReactFlowProp = !!("onMove" ==> flowTransform)

    static member inline onMoveStart(flowTransform: unit) : IReactFlowProp = !!("onMoveStart" ==> flowTransform)

    static member inline onMoveEnd(flowTransform: unit) : IReactFlowProp = !!("onMoveEnd" ==> flowTransform)

    static member inline onDrop(handler: Event -> Node [] -> unit) : IReactFlowProp = !!("onDrop" ==> handler)
    ...
    static member inline onPaneClick(handler: Event -> unit) : IReactFlowProp = !!("onPaneClick" ==> handler)

    static member inline onPaneContextMenu(handler: Event -> unit) : IReactFlowProp =
        !!("onPaneContextMenu" ==> handler)

    static member inline onPaneScroll(handler: Event -> unit) : IReactFlowProp = !!("onPaneScroll" ==> handler)
